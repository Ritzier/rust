...
/// Parse a list of variable names separated by commas.
///
///     a, b, c
///
...
impl Args {
    ...
    /// Procues an expression that assings the right-hand side to the left-hand
    ...
    /// Procudues a let-binding that assigns the right-hand side to the left-hand
    /// side and then prints the value.
    ///
    ///     // Before
    ///     let VAR = INIT;
...
/// simply recurse on my child nodes. We can override only those methods for
/// which we want non-default behavior. In this case the traversal need to
...
/// Attribute to print hte value of the given variables each time they are
/// reassigned.
///
/// # Example
///
...
pub fn trace_var(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // Parse the list of varaibles the user wanted to print.
    let mut args = parse_macro_input!(args as Args);

    // Use a syntax tree traversal to transform the function body.
    let output = args.fold_item_fn(input);

    // Hand the resulting body back to the compiler.
