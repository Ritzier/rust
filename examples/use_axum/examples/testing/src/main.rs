...
/// without having a create an HTTP server.
fn app() -> Router {
    Router::new()
...
mod tests {
    ...
    async fn hello_world() {
        ...
        // call it like any tower service, no need to run all HTTP server.
        let response = app
    ...
    // You can use `read()` and `call` to avoid using `clone()`
    // in multiple request
    #[tokio::test]
    ...
    //
    // This is normally set with `Router::into_make_service_with_connect_info` but we can't easily
