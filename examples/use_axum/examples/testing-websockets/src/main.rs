...
async fn main() {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:3000")
        .await
        .unwrap();
    println!("listening oin {}", listener.local_addr().unwrap());
...
fn app() -> Router {
    ...
    //
    // - Intergration tests where you run the server and connect with a real WebSocket client.
    // - Unit tests where you mock teh socket as some generic send/receive type
    //
    // Which version you pick is up to you. Generally we recommend the intergration test version
    // unless your app has a lot of setup that makes it hard to run in a test.
    Router::new()
        .route("/integration-testable", get(intergration_testable_handler))
        .route("/unit-testable", get(unit_testable_handler))
}

// A WebSocket handler that echos any message it receives.
//
// This one we'll be intergration tsting so it can be written in the regular way.
async fn intergration_testable_handler(ws: WebSocketUpgrade) -> Response {
    ws.on_upgrade(intergration_testable_handle_socket)
}

async fn intergration_testable_handle_socket(mut socket: WebSocket) {
...
// The implementation is largely the same as `intergration_testable_handle_socket` expect we call
...
mod tests {
    ...
    #[tokio::test]
    async fn unit_test() {
        // Need to sue "futures" channels rather than "tokio" channels as they implement `Sink` and
        // `Stream`
