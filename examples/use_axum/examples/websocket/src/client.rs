...
const N_CLIENTS: usize = 10; // set a desired number
const SERVER: &str = "ws://127.0.0.1:3000/ws";

#[tokio::main]
async fn main() {
    let start_time = Instant::now();
    // spawn several clients taht will concurrently talk to the server
    ...
    // total time should be the same no matthe how many clients we spawn
    println!(
        "Total time taken {:#?} with {N_CLIENTS} concurrent clients, should be abut 6.45 seconds.",
        end_time - start_time
    )
}

// creates a client. quietlky exits on failure.
async fn spawn_client(who: usize) {
    let ws_stream = match connect_async(SERVER).await {
        Ok((stream, response)) => {
            println!("Handshake for client {who} has been coimpleted");
...
fn process_message(msg: Message, who: usize) -> ControlFlow<(), ()> {
    match msg {
        ...
        // v according to spec. Buf if you need the contents of the pings you can see them here.
        Message::Ping(v) => {
