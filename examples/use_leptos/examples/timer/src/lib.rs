...
pub fn TimerDemo() -> impl IntoView {
    // count_a updates with a fixed interval of 1000 ms, hwereas cunt_b has a dynamic
    // update interval
    let count_a = RwSignal::new(0_i32);
    let count_b = RwSignal::new(0_i32);

    let interval = RwSignal::new(1_000);
    ...
    view! {
        <div>
            <div>"Count A (fixed intarval of 1000ms)"</div>
            <div>{count_a}</div>
...
/// Hook to wrap the underlying `setInterval` call and make it reacitve w.r.t.
    ...
    T: Into<Signal<u64>> + 'static,
{
    ...
    Effect::new(move |prev_handle: Option<IntervalHandle>| {
        ...
        .expect("count not create interval")
    });
}
