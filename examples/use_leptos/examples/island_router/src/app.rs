...
fn Home() -> impl IntoView {
    ...
    let view = move || {
        Suspend::new(async move {
            ...
            match q().is_none() {
                true => EitherOf3::A(
                    view! { <p class="note">"Enter a search to begin viewing contacts."</p> },
                ),
                false if users.is_empty() => EitherOf3::B(
                    view! { <p class="note">"No users found matching that search."</p> },
                ),
                false => EitherOf3::C(view! {
                    <table>
                        <tbody>
...
pub fn Details() -> impl IntoView {
    ...
    pub async fn get_user(id: u32) -> Result<Option<User>, ServerFnError> {
        ...
        Ok(data.into_iter().find(|user| user.id == id))
    }
...
pub fn About() -> impl IntoView {
    view! {
        <section class="page">
            <h2>"About"</h2>
            <p>
                "This is demo is intended to shaw off an experimental \"islands router\" feature, which mimics the smooth transitions and user experience of client-side routing while minimizing the amount of code that actually runs in the browser. "
            </p>
            <p>
                "By default, all the content in this application is only rendered on the server. But you can add client-side interacting via islands like this one:"
            </p>
            <Counter />
        </section>
    }
...
fn Counter() -> impl IntoView {
    ...
    view! {
        <button class="counter" on:click=move |_| *count.write() += 1>
            {count}
        </button>
    }
}
...
pub async fn search(query: String) -> Result<Vec<User>, ServerFnError> {
    ...
    Ok(data
        .into_iter()
        .filter(|user| {
            user.first_name.to_ascii_uppercase().contains(&query)
