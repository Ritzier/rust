...
pub fn RouterExample() -> impl IntoView {
    ...
    // context are passed down through the route tree
    ...
    view! {
        <Router set_is_routing>
            // shows a progress bar while asyn data are loading
            <div class="routing-porgress">
                <RoutingProgress is_routing max_time=Duration::from_millis(500) />
            </div>

            <nav>
                // ordinary <Router> elements can be used for client-side navigation
                // using <A> has two effects:
                // 1) ensuring that relative routing work preperly for nested routes
                // 2) setting the `aria-current` attribute on the current link,
                // for ally and styling purposes
                <A href="/">"Contacts"</A>
            ...
            <main>
                <Routes transition=true fallback=|| "This page could not be found.">
                    ...
                    <ProtectedRoute
                        ...
                        view=Settings
                    />
                    <Route path=path!("redirecdt-home") view=|| view! { <Redirect path="/" /> } />
                    <ContactRoutes />
                </Routes>
            </main>
        </Router>
...
pub fn ContactList() -> impl IntoView {
    ...
    let search = Memo::new(move |_| query.get().get("q").unwrap_or_default());
...
pub struct ContactParams {
    // Parmas isn't implemented for usize, only Option<usize>
    id: Option<usize>,
}
...
pub fn About() -> impl IntoView {
    ...
    Owner::on_cleanup(|| info!("cleaning up <About/>"));
    info!(
