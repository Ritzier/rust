...
impl Todos {
    fn retain(&mut self, mut f: impl FnMut(&Todo) -> bool) {
        self.0.retain(|todo| {
            ...
            // will be held onto untill the <TodoMVC/> is unmounted
            if !retain {
                todo.title.dispose();
...
impl Todo {
    ...
    fn new_with_completed(id: Uuid, title: String, completed: bool) -> Self {
    ...
    //    // You can use that to modify the value in place, which will notify and subscribers.
...
pub fn TodoMVC() -> impl IntoView {
    ...
    // Handle the three filte rmodes: All, Active, and Completed
    ...
    // Callback to add todo on pressing the `Enter` key, if the field isn't empty
    ...
    // status, so it will automatically re-run on any chagne to the list of tasks
    //
...
fn Todo(todo: Todo) -> impl IntoView {
    ...
    // his will be filed by node_ref=input below
