    ...
    view! {
        ...
        <html lang="en">
            <head>
                ...
                <link rel="stylesheet" id="leptos" href="/pkg/ssr_mode_axum.css" />
                <MetaTags />
            </head>
            <body>
                <App />
...
#[server]
async fn set_is_admin(is_admin: bool) -> Result<(), ServerFnError> {
...
pub fn App() -> impl IntoView {
    ...
    let is_admin = Resource::new(move || toggle_admin.version().get(), |_| is_admin());
    ...
    view! {
        ...
        <Router>
            ...
            <main>
                <FlatRoutes fallback>
                    // Load the home page with out-of-order streaming and <Suspense/>
                    <Route path=StaticSegment("") view=HomePage />

                    // Load the posts with async rendering, so they can set
                    // the title and metadat *after* loading the data
                    <Route
...
fn Post() -> impl IntoView {
    ...
    let comments_resource = Resource::new(id, |id| async move {
        match id {
            Err(e) => Err(e),
            Ok(id) => get_comments(id).await.map_err(|_| PostError::ServerError),
        }
    });
        ...
        match post_resource.await {
            Ok(Ok(post)) => {
                Ok(view! {
                    ...
                    // this metadata should be included in the actuall HTML <head>
...
#[server]
async fn list_post_metadata() -> Result<Vec<PostMetadata>, ServerFnError> {
...
#[server]
async fn get_comments(id: usize) -> Result<Vec<String>, ServerFnError> {
